...
把一段含时间和事件的文字交给 AI，自动拆成短条目，并按日期显示在时间轴上。也可以不经过 AI，直接在当前日期手动添加或删除条目。

适合个人每天多次快速整理日程。界面为中文。数据保存在当前浏览器的 localStorage。

## 功能

- 一次性粘贴日程原文，调用 OpenAI 兼容接口解析
- 按日期展示时间轴，默认今天，可前后切换或点选日期
- 在当前日期手动添加日程：填写时间和标题后加入时间轴
- 删除单条日程，立即从时间轴和本地存储中移除
- 同一天的新解析结果覆盖该日旧条目，其他日期保留；手动添加不会覆盖已有条目
...
Pages 控制台只需要构建命令和输出目录，不要填 Workers 的「部署命令 / 版本命令」。

部署完成后访问 `https://<项目名>.pages.dev`。前端请求 `/api/parse` 时，由 Pages Functions 转发到你在页面里填写的模型接口。

如果走的是 Workers 构建表单（含构建命令、部署命令、版本命令），按下面填写：

| 字段 | 值 |
|------|-----|
| 构建命令 | `npm run build` |
| 部署命令 | `npx wrangler pages deploy dist` |
| 版本命令 | `npx wrangler pages deploy dist` |
...
1. 先切换到要添加的日期
2. 在时间轴上方填写「时间」和「日程」标题
3. 点击「添加日程」，或在标题输入框按回车

手动添加会插入当前日期的已有列表，不会覆盖当天其他条目。无需配置 API。

### 删除一条日程

每条日程右侧有「删除」按钮。点击后该条会从时间轴和浏览器本地存储中移除。
